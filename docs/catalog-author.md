...
├── ssh/id_ed25519.pub          ← generated by instructor bootstrap, never committed
└── labs/
...
| File | Read by | Holds |
| --- | --- | --- |
| `README.md` | `dsoxlab course`, in one block with `scenario.md` unless a `course.yaml` splits it | the lesson: what the lab teaches, and what it assumes known |
| `scenario.md` | `dsoxlab course` | the situation: where the learner is and what is wrong — not the steps that fix it |
| `challenge/README.md` | `dsoxlab challenge`; printed by `run` before the SSH session of a `vm` lab opens, since the host has no dsoxlab; read by `validate-structure` for the scale | the mission, as a list of graded tasks |
...
`test_functional.py` means twenty points each, hints deducted afterwards. The
mission announces that scale, and `validate-structure` holds the two together —
adding one test to a lab silently reweights every task, and this check is the
...
A `### ` heading carrying `(N pts)` or `(N points)` is a graded task; a line
naming *N tasks* and *M points* is the announcement. `##` headings are not
counted, so an exam paper can group its tasks into sections without doubling
its own total. The validator fails when the tasks do not sum to the announced
total (`content_scoring_points_mismatch`), when their count differs from the
announced one (`content_scoring_count_mismatch`), or from the number of tests
(`content_scoring_tasks_vs_tests`). A mission that announces no per-task points
is not checked at all: a mock exam verifying several things per task made
another choice, just as valid. `challenge/README.fr.md` is read first when it
exists.

**The hints.** `challenge/hints.yaml` carries the scale and the hints, in the
order they will be handed out:

```yaml
points: 100            # the lab's scale; 100 when the file is absent
hints:
  - text_en: "The group must exist before the user does."
    text_fr: "Le groupe doit exister avant l'utilisateur."
    cost: 10           # points removed when the learner takes it (default 10)
```

A legacy single `text` key is still read, and a base64 value is decoded so a
hint does not surface in a `git grep`; neither is needed in a new lab.

**The tests.** pytest runs from the **catalog root**, so a `conftest.py` there
is collected for every lab: that is where the `build_inventory` import below
belongs, and where a catalog of `shell` labs resolves `<lab>/challenge/work`
from the location of the test file. `dsoxlab new catalog` writes none; the
Linux and Terraform catalogs each carry one at their root, and either is a
